# LiteStartup Dokumentation

Professionelle Dokumentation für LiteStartup - Intelligente E-Mail-Marketing-Plattform für Startups.

## Überblick

LiteStartup ist eine SaaS-E-Mail-Marketing-Plattform, die speziell für Startups entwickelt wurde. Sie kombiniert geschäftliche E-Mails (Workmail), KI-gestützte Inhaltserstellung und zuverlässige E-Mail-Zustellung in einer erschwinglichen, benutzerfreundlichen Plattform.

## Inhalt der Dokumentation

- **[Einführung](00-introduction.md)** - Erfahren Sie, was LiteStartup ist und warum Sie es verwenden sollten
- **[Erste Schritte](01-getting-started.md)** - Registrieren Sie sich und senden Sie Ihre erste E-Mail in 5 Minuten
- **[API-Referenz](02-api-reference.md)** - Vollständige API-Dokumentation und Endpunkte
- **[Funktionsübersicht](03-features.md)** - Erkunden Sie alle Funktionen im Detail
- **[Code-Beispiele](04-examples.md)** - Implementierungsbeispiele in mehreren Sprachen
- **[Preise & Pläne](05-pricing.md)** - Verstehen Sie Preise und Abrechnung

## Schnellzugriff

- [LiteStartup Website](https://www.litestartup.com)
- [Kostenloses Konto erstellen](https://app.litestartup.com/signup)
- [Dashboard](https://app.litestartup.com/dashboard)

## Hauptmerkmale

✓ **Workmail** - Integrierte geschäftliche E-Mail (kein Google Workspace erforderlich)

✓ **KI-Assistent** - Erstellen Sie Betreffzeilen und E-Mail-Inhalte

✓ **Automatisierung** - Intelligente Workflows, die durch Benutzerverhalten ausgelöst werden

✓ **Erschwinglich** - Bezahlen Sie pro gesendeter E-Mail, nicht pro Kontakt

✓ **Einfache Integration** - Einfache REST-API, funktioniert mit jeder Sprache

✓ **Zuverlässigkeit für Unternehmen** - 99,99% Verfügbarkeit, SOC2- & GDPR-konform

## Erste Schritte

1. **[Lesen Sie die Einführung](00-introduction.md)** um die Plattform zu verstehen
2. **[Folgen Sie dem Leitfaden für erste Schritte](01-getting-started.md)** um Ihre erste E-Mail zu senden
3. **[Prüfen Sie die API-Referenz](02-api-reference.md)** für technische Details
4. **[Sehen Sie sich Code-Beispiele an](04-examples.md)** für Ihre Programmiersprache
5. **[Erkunden Sie Funktionen](03-features.md)** um erweiterte Funktionen kennenzulernen

## Support

- 📖 Schauen Sie in unsere [umfassende Dokumentation](00-introduction.md)
- 💬 Besuchen Sie [LiteStartup.com](https://www.litestartup.com) für Support
- 🐛 Melden Sie Probleme auf GitHub
- 📧 Kontakt: support@litestartup.com

---

**Bereit loszulegen?** [Erstellen Sie noch heute Ihr kostenloses Konto](https://app.litestartup.com/signup)!
